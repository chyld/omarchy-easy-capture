#!/bin/bash
# Easy Capture's local copy of Omarchy's installed omarchy-capture-region.
# Copyright (c) David Heinemeier Hansson. See LICENSE.omarchy.
# Adapted to offer visible windows across ALL enabled monitors. The picker,
# freeze protocol, cancellation and selection markers follow the stock helper.
# Upstream source: https://github.com/omacom/omarchy/blob/master/bin/omarchy-capture-region
...
# Keep a window only when its workspace is visible on its own monitor.
# Pinned windows and an open special workspace are visible too.
window_rects() {
  hyprctl clients -j | jq -r --argjson monitors "$(hyprctl monitors -j)" '
    [.[] | select(.hidden != true and .mapped != false) | . as $client |
      select(any($monitors[];
        .disabled != true and .id == $client.monitor and
        ($client.pinned == true or .activeWorkspace.id == $client.workspace.id or
          ((.specialWorkspace.id // 0) != 0 and .specialWorkspace.id == $client.workspace.id)))) |
...
# Read-only diagnostic used to verify the picker candidates.
if [[ ${1:-} == "--list-rectangles" ]]; then
  get_rectangles
  exit 0
fi
